...
class ChangelLifecycleTests(unittest.TestCase):
    ...
    def test_team_only_evidence_after_expiry_releases_full_bond_to_team(self):
        promise_id=self.create(); self.sender(self.team)
        self.contract.submit_release_evidence(promise_id,"https://raw.githubusercontent.com/acme/widget/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/README.md","Release evidence")
        self.gl.message_raw["datetime"]="2027-01-01T00:00"; self.sender(self.challenger)
        self.contract.settle_expired_single_party_evidence(promise_id)
        record=json.loads(self.contract.get_promise(promise_id))
        self.assertEqual(record["status"], "settled_single_party_evidence")
        self.assertEqual(record["outcome"], "team_only_evidence")
        self.assertEqual(TRANSFERS, [(self.team,100)])
    def test_challenger_only_evidence_after_expiry_releases_full_bond_to_challenger(self):
        promise_id=self.create(); self.sender(self.challenger)
        self.contract.submit_counter_evidence(promise_id,"https://raw.githubusercontent.com/acme/widget/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/README.md","Counter evidence")
        self.gl.message_raw["datetime"]="2027-01-01T00:00"; self.sender(self.team)
        self.contract.settle_expired_single_party_evidence(promise_id)
        record=json.loads(self.contract.get_promise(promise_id))
        self.assertEqual(record["status"], "settled_single_party_evidence")
        self.assertEqual(record["outcome"], "challenger_only_evidence")
        self.assertEqual(TRANSFERS, [(self.challenger,100)])
